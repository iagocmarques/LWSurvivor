...
{
    public static class Lf2Wave72Verifier
    {
        ...
        [MenuItem("_Project/LF2/Verify Wave 7.2 (Woody, Dennis, Freeze)")]
        public static void VerifyWave72()
        {
            ...
            foreach (var kv in TargetCharacters)
            {
                ...
                {
                    Debug.LogError($"[Wave72] {kv.Value} not found at {datPath}");
                    allOk = false;
                    continue;
                }
                ...
                {
                    Debug.LogError($"[Wave72] {kv.Value}: parse failed or 0 frames");
                    allOk = false;
                    continue;
                }

                Debug.Log($"[Wave72] === {data.Name} (id={kv.Key}, file={kv.Value}) ===");
                ...
                // Validate critical frames exist
                ...
                // Check specials have valid frame IDs (not defaults when character should have them)
                ...
                // List all frames with itrs (attacks)
            ...
            if (db == null)
            {
                Debug.Log("[Wave72] Database asset not found. Running batch import...");
            ...
            if (db != null)
            {
                Debug.Log($"[Wave72] Database has {db.Count} characters");
                foreach (var kv in TargetCharacters)
                {
            ...
            {
                Debug.LogError("[Wave72] Failed to create/load database!");
                allOk = false;
            }

            if (allOk)
                Debug.Log("[Wave72] ALL 3 CHARACTERS VERIFIED SUCCESSFULLY");
            else
                Debug.LogWarning("[Wave72] SOME ISSUES FOUND - see warnings above");
        }
